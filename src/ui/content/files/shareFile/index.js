...
export class SharePrompt extends React.Component {
  constructor(props) {
    super(props);
    this.state = { currentPage: 1, isLoaded:false,link:"" };
  ...
  componentDidMount = async ()=>{
    ...
    if(fjson.Message=="Create"){
        ...
        var key = await importAndDeriveKeyFromRaw(localStorage.getItem("key"),Ab.slice(16,32))
        var dec = new TextDecoder()
        this.setState({link: "https://s.crypithm.com/"+dec.decode(await decryptBlob(key, Ab.slice(0,16), Ab.slice(32)))})
    }
  }
  unMount = () => {
  ...
  genLink = async() =>{
    ...
    this.setState({link: await shareFile(this.fileProps.SharedId, this.fileProps.Key, this.fileProps.Tk, checked, this.props.name)})
    this.setPage(2)
  }
  checkboxChanged=()=>{
        ...
        {this.state.currentPage == 1 ? (
            ...
            <div className="share-prompt">
              ...
              <div className="body">
                <div className="option">
                  <input type="checkbox" onChange={this.checkboxChanged} ref={this.displayUserRef}/>
                  <div className="exp">
          ...
          <>
            ...
            <div className="share-prompt">
              ...
              <div className="body">
                <textarea className="link-area" readOnly rows="1" defaultValue={this.state.link}>
                </textarea>
                <div className="btns">
                  <div className="cancel-btn" onClick={this.unMount}>
                    Cancel
                  </div>
                  <div className="share-btn">Copy</div>
                </div>
              </div>
            </div>
          </>
